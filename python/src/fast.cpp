...
void init_fast(nb::module_& parent_module) {
  ...
  m.def(
      ...
      "stream"_a = nb::none(),
      nb::sig(
          "def quantized_scaled_dot_product_attention(q: array, k: tuple[array, array, array], v: tuple[array, array, array], *, scale: float,  mask: Optional[array] = None, group_size: int = 64, bits: int = 4, stream: Union[None, Stream, Device] = None) -> array"),
      R"pbdoc(
        A fast implementation of multi-head attention: ``O = softmax(Q @ K.T, dim=-1) @ V``.
